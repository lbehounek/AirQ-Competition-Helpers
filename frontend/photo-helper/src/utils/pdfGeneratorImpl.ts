// Heavy PDF implementation (@react-pdf/renderer + pdfkit/fontkit/yoga/… ≈1.6 MB
// minified). NEVER import this module statically from app code — it is reached
// only through `./pdfGenerator`'s `import()`, which is what keeps it in its own
// Rollup async chunk (`pdfGeneratorImpl-*.js`) instead of the eager entry
// bundle. The build guard in `frontend/vite.chunks.ts` (`assertLazyOnly`) fails
// `vite build` if @react-pdf ever becomes statically reachable from the entry.
...
import { acquireDedicatedWebGLContext } from './webglContextManager';
...
/**
 * Progress report emitted while an export runs.
 * `done`/`total` count photo CELLS across both pages (placeholders included);
 * they are only meaningful during the `render` phase, and `done === total`
 * from `compose` onwards.
 */
export interface PdfProgress {
  phase: 'render' | 'compose' | 'save';
  done: number;
  total: number;
}

/** Optional extras for {@link generatePDF}; everything here is opt-in. */
export interface GeneratePdfOptions {
  onProgress?: (progress: PdfProgress) => void;
}

/**
 * Encode a canvas as JPEG for embedding in the PDF.
 * Resolves to a `Blob` on the happy path and to a data-URL string on any
 * fallback; never rejects, so a cell can always be produced.
 *
 * WHY not plain `toDataURL`: that is a SYNCHRONOUS main-thread encode (≈30–100
 * ms for 1600x1200 on a low-end laptop) that then inflates to a ~700 KB base64
 * string which @react-pdf immediately decodes again with `Buffer.from(…,
 * 'base64')`. `toBlob` is asynchronous and @react-pdf accepts `image/jpeg`
 * Blobs directly. A null blob (tainted or zero-sized canvas) or a `toBlob`
 * that throws falls back to the old data URL, so this is never worse than
 * before.
 */
const canvasToJpegSrc = (canvas: HTMLCanvasElement): Promise<Blob | string> =>
  new Promise((resolve) => {
    try {
      canvas.toBlob(
        (blob) => resolve(blob ?? canvas.toDataURL('image/jpeg', PDF_PHOTO_JPEG_QUALITY)),
        'image/jpeg',
...
// Header rasterisation is deliberately NOT cached and deliberately still uses
// the synchronous `toDataURL`. Measured cost: `createTextImage` runs at most
// twice per page (title + promo, and promo is off while BRANDING_ENABLED is
// false), each ≈90×1200–2400 px at 3× ≈ 0.1–0.2 MP, i.e. ≈1–5 ms of fillText +
// PNG encode — under 20 ms for a whole export, well below 0.5 % of a 10-photo
// run. A cache would also almost never hit: set1 and set2 headers differ in
// text. Photo labels do not come through here at all — `drawLabel` writes them
// straight onto the photo canvas.
// Add when: a profile shows header rasterisation mattering — memoize by
// `${text}|${rotated}|${fontSize}` in a Map local to one export run.
...
/**
 * The actual export. Not exported — callers go through `generatePDF`, which
 * adds the in-flight guard. Resolves when the PDF has been saved/downloaded;
 * rejects (with `renderFailures` attached) when any photo cell could not be
 * rendered at all.
 */
const runGeneratePdf = async (
  ...
  // Private WebGL context for this export. See `acquireDedicatedWebGLContext`
  // for why the editor's shared pool must not be borrowed here. `null` when
  // WebGL is unavailable — `renderPhotoOnCanvas` then takes its CPU path,
  // exactly as every export did before WP2d.
  const dedicatedGl = acquireDedicatedWebGLContext();

  // Per-photo render result. `src` is whatever @react-pdf should embed: a
  // JPEG Blob normally, a data URL when the Blob encode was unavailable.
  // `degraded` flags the case where the hi-res
  ...
  type PhotoRenderResult =
    | { kind: 'ok'; src: Blob | string }
    | { kind: 'degraded'; src: Blob | string; photoId: string }
  ...
  const getPhotoDataUrl = async (photo: ApiPhoto): Promise<PhotoRenderResult> => {
    ...
    if (photo.isPlaceholder) {
      ...
      return { kind: 'ok', src: await canvasToJpegSrc(canvas) };
    }
    try {
      ...
      // Render with the SAME shader the live preview uses, via this export's
      // private context (8th arg). That is what makes the printed sheet match
      // what the user tuned in the editor: the CPU path this used to force is
      // a separate implementation with a different white-balance gain, an
      // inverted tint direction and a different sharpen kernel, so the sheet
      // never looked like the preview.
      // `dedicatedGl` is null when WebGL is unavailable, and
      // `applyWebGLEffects` returns null on a lost context, an oversize source
      // or a downsized drawing buffer — all of which fall through to
      // `renderPhotoOnCanvas`'s CPU path exactly as before.
      // The 9th arg (webglManager) stays undefined so the export can never
      // borrow — and resize — one of the editor grid's pooled contexts.
      await renderPhotoOnCanvas(
        ...
        true,
        mode,
        undefined, // no shared scratch buffer: one render per photo, then gone
        // Bypass the resize cache. 1600 px × this photo's zoom is a size the
        // editor never asks for, so an entry buys nothing and evicts the
        // large-modal entries the cache exists for — at the price of a
        // full-size canvas clone per photo. Per-call, NOT a process-wide
        // suspend: the export yields between photos, so the user can open the
        // modal and drag a slider while it runs, and those redraws must still
        // be cached.
        true,
      );
      ...
      try {
        ...
        if (canvasElement) {
          return {
            kind: 'degraded',
            src: await canvasToJpegSrc(canvasElement),
            photoId: photo.id,
          };
        }
      } catch {
  ...
  /**
   * The photos a page will actually render, i.e. `photos` truncated to the
   * grid's cell count. Returns a slice (possibly containing holes on
   * defensive data), never a copy of the whole set.
   *
   * Safe to call with the DEFAULT header extents even though `createPage`
   * re-measures them: `rows`/`cols` depend only on `layoutMode` (portrait is a
   * fixed 2×5) and `pageCount` (landscape switches to 5×2 at 10 photos, see
   * `pdfLandscapeGrid.ts`). A measured header only moves gutters and cell
   * sizes, never the grid shape — so this is exactly the set `createPage`
   * renders, which is what makes the progress total correct before either page
   * is built.
   */
  const photosToRenderFor = (photoSet: ApiPhotoSet): ApiPhoto[] => {
    const { rows, cols } = calculateLayout(15, 25, 2.83, photoSet.photos.length);
    return photoSet.photos.slice(0, rows * cols);
  };
  ...
  const createPage = async (photoSet: ApiPhotoSet, setTitle: string, pageKey: string) => {
    ...
    const photosToRender = photosToRenderFor(photoSet);

    // Decode every source image up front, overlapped. Only the DECODE is
    // parallel — the renders below are sequential (see why immediately after).
    // A per-photo load error is deliberately swallowed here; the render loop
    // asks the cache again inside `getPhotoDataUrl`, where a failure is
    // classified and surfaced as it always was.
    await Promise.all(
      photosToRender
        .filter((p) => p && !p.isPlaceholder)
        .map((p) => getImageCache().getImageByUrl(p.url).catch(() => null)),
    );

    // Render sequentially, yielding to the event loop after each photo.
    // WHY not `Promise.all`: each render is a long SYNCHRONOUS chunk (effects
    // + JPEG encode) and running them "in parallel" only interleaved their
    // await points — the window still froze for the whole export and no
    // progress could ever paint. One photo at a time plus a macrotask hop
    // gives the browser a rendering opportunity between cells at a cost of
    // ≈1–4 ms each.
    const renderResults: (PhotoRenderResult | null)[] = [];
    for (const p of photosToRender) {
      if (!p) {
        renderResults.push(null);
        continue;
      }
      // DEV-only User Timing marks, kept OUTSIDE the failure-classifying
      // try/catch inside `getPhotoDataUrl` so instrumentation can never turn a
      // good cell into a failed one. Guarded by typeof as well as DEV because
      // not every runtime this file is loaded in has User Timing.
      const canMark =
      ...
      if (canMark) {
        try {
          performance.mark(`pdf-photo-${p.id}-start`);
        } catch { /* instrumentation must never fail a cell */ }
      }

      renderResults.push(await getPhotoDataUrl(p));

      if (canMark) {
        try {
          performance.measure('pdf-photo-render', `pdf-photo-${p.id}-start`);
        } catch { /* instrumentation must never fail a cell */ }
      }
  ...
  // Progress accounting. `total` is every cell both pages will render,
  // computed BEFORE any rendering starts so the UI can show "3 of 9" from the
  // first report. `filter(Boolean)` is a no-op on well-formed `ApiPhoto[]` but
  // mirrors the render loop's hole handling, so `done === total` still holds
  // if a set ever arrives with a sparse array.
  const total = [set1, set2]
    .filter((s) => s.photos.length > 0)
    .reduce((n, s) => n + photosToRenderFor(s).filter(Boolean).length, 0);
  let done = 0;
  /** Emit one progress tick; a caller without `onProgress` costs nothing. */
  const report = (phase: PdfProgress['phase']) => options.onProgress?.({ phase, done, total });

  try {
    // The cache bypass rides on each `renderPhotoOnCanvas` call instead of a
    // process-wide flag around this block — see the `true` argument at the
    // hi-res render, and why, there.
    ...
    // Release the GPU context the moment the last photo is rendered. It can be
    // holding a ~69 MB drawing buffer, and everything below — PDF composition
    // and especially the native save dialog — can take minutes. The `finally`
    // below stays as the safety net; `dispose` is idempotent.
    dedicatedGl?.dispose();
    ...
    try {
      // Composition is one long synchronous-ish chunk with no interior yield
      // points of ours, so tell the UI what it is waiting on and give the
      // progress commit a chance to paint before we enter it.
      report('compose');
      await yieldToEventLoop();
      ...
      // From here on the user may be looking at a native save dialog, which
      // can stay open indefinitely — the caption should say "saving", not
      // "composing".
      report('save');
    ...
    // Safety net: the eager dispose above only runs when rendering got
    // that far. Any throw before it (a render failure abort, a compose or
    // save error) must still release the context — `dispose` is idempotent,
    // so the double call on the happy path is free.
    dedicatedGl?.dispose();
  }
};

/**
 * In-flight export, or null. Module-level on purpose: a single PDF export is a
 * whole-application operation (one save dialog, one working-directory write),
 * not something a component instance owns.
 */
 ...
 *
 * Resolves once the PDF has been saved (Electron) or downloaded (web).
 * Rejects with an `Error` carrying `renderFailures` when one or more photo
 * cells could not be rendered at all — the caller is expected to surface that.
 *
 * A second call made WHILE an export is running returns the SAME promise
 * instead of starting another one. WHY this is now required: the export yields
 * to the event loop between photos, so the Generate button is clickable
 * mid-run; without coalescing, a second click would mean two dedicated GL
 * contexts, two `pdf().toBlob()` passes, two native save dialogs and two
 * racing `setWorkingDir` writes. The coalesced caller shares the first call's
 * outcome; its `onProgress` is deliberately not wired up (the first caller
 * owns the progress UI).
 *
 * All nine original positional parameters are unchanged; `options` is a tenth,
 * optional one, so existing call sites keep compiling untouched.
 */
