...
import { createSerialQueue } from '@airq/shared-storage'

// The ordering primitive behind photo-helper's competitionService write queue
// (and, in map-corridors, the session writer and photo-import queue). It lives
// in @airq/shared-storage, which has no vitest setup of its own — so its test
// lives here, the same way `dirnameOf.test.ts` does.
//
// Everything here is deliberately deterministic (hand-rolled deferreds,
